...
- **Baselines:** `data/baselines/{industry}/{stream}.ttl` + `.bpmn` — generated by `uv run ots-ingest` / the [industry-standards agent](#agents) from APQC PCF v8 (generic + 11 industry PCFs) and TM Forum eTOM (telecom), with provenance, `ots:precedes` flow edges, and function-unit tags. **13 industries** shipped; kept current by the agent
...
OTS has two kinds of agent.

### Reference-data agent — keeping the business processes up to date

The **industry-standards agent** (`services/ingest/industry_agent`, CLI
`ots-industry-agent`) keeps the shared reference process baselines aligned with the
source APQC industry PCF standards. When a standards body publishes a new PCF version
(a refreshed PDF is added under `ReferenceDocs/Industries/`), the agent detects the
change and regenerates that industry's baselines. It is deterministic — no LLM needed.

```bash
uv run ots-industry-agent list                 # discovered industry PDFs
uv run ots-industry-agent check                # drift report; exits non-zero when a PCF changed
...
Per industry it: parses the PDF → proposes a value-stream → PCF-subtree mapping
(draft `streams_{slug}.yaml`) → derives the engagement profile (`data/profiles/{slug}.json`)
→ emits OWL TTL + BPMN + SKOS thesaurus → validates. It never overwrites a human-reviewed
mapping (files without the `AGENT-GENERATED DRAFT` marker) and emits from the on-disk
YAML, so curated overrides win. A content-hash manifest (`data/baselines/.industry_manifest.json`)
makes `check`/`sync` idempotent, so it can **run periodically** (cron, CI open-PR-on-drift,
or a Claude Code routine) to track evolving standards. **13 industries** are kept in sync
today; generic (APQC) and telecom (eTOM) refresh through the same `ots-ingest` pipeline the
agent reuses. Details: [`services/ingest/industry_agent/README.md`](services/ingest/industry_agent/README.md).

### Engagement drafting agents (LLM)

In-app, LLM-backed agents draft engagement-specific artefacts for a consultant to verify
