...
**Status:** UI iterations **A–E complete** (mock-first frontend + live Fuseki/FastAPI for ontology). Post-E infrastructure (Postgres, audit, PDF, real connectors, SSO, E2E) is next.

Human-driven engagements with typed service contracts. Ontology reads/writes go through FastAPI → Fuseki; everything else uses mock services until Postgres lands.
...
**After E:** audit trail, Postgres, PDF export, real Salesforce/Jira APIs, live LLM gap analysis, SSO, Playwright E2E.
...
Next.js (apps/web)  →  FastAPI (services/api)  →  Fuseki (OWL/RDF)
        │
        └── mock services (engagements, BPMN, teleology, connectors, review, AI gaps)
```
...
- **Baselines:** `data/baselines/{stream}.ttl` — OWL classes with `rdfs:subClassOf` hierarchy and `ots:precedes` process-flow edges
