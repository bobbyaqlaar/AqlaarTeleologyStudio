...
- 2026-07-15 (Actor–Method process model, Phase 3.4): Input-side errors one-click too. Input problems carry `compatible` (type-satisfying existing variables) + `expectedLabel`; card renders "Bind to '…'" per compatible var + "Initialise global (Type)" (creates a global of the expected concept + binds). Verified in-app (unbound → Bind to 'cust' → green Consistent) + API (initialise-global → valid). `tsc` clean.

- 2026-07-15 (Actor–Method process model, Phase 3.3): Corrections actionable in GUI. Output bindings editable inline; each output problem carries its `output` param; problem card shows one-click "Apply fix: rename output to '…'" (rebinds to a free name from the used-name set). Verified in-app: output_overwrite warning → Apply fix → output becomes `order_2` → green Consistent. Keyed binding inputs to committed value (remount) to clear a Base UI uncontrolled-input warning + stale-display. `tsc` clean.

- 2026-07-15 (Actor–Method process model, Phase 3.2): Problem severity error|warning. Compatible output overwrite of an existing variable → `output_overwrite` warning (silent-overwrite guard); incompatible stays `output_conflict` error. `/validate` → `{valid, errors, warnings, problems}` (warnings don't invalidate). UI amber-vs-red: header badge, Apply banner, per-step cards by severity. Verified API + in-app (amber "1 warning" badge/banner/card). `tsc` clean.

- 2026-07-13 (Actor–Method process model, Phase 3.1 polish): (a) Explicit "Apply & check" button (any time) → `/validate`, pass/fail banner; validation extended to check OUTPUTS — `output_unbound` (output not bound to a variable) + `output_conflict` (re-publishing a variable with an incompatible ontology type), verified via API. (b) BPMN view — Steps ⇄ BPMN header toggle; BPMN renders the generated diagram full-size (read-only `BpmnEditor`). Verified in-app (green consistency banner; lane diagram). `tsc` clean.

- 2026-07-13 (Actor–Method process model, subtype + Phases 2–3): (a) Ontology subtype compatibility — `FusekiClient.ancestor_map` (subClassOf+/broader+), `is_a` now polymorphic; verified subtype satisfies ancestor-typed input, unrelated → mismatch. (b) Phase 2 seed importer `seed-from-baseline` (baseline BPMN → actors+methods+steps, idempotent); generic O2C → 18 steps. (c) Phase 3 workshop UI: `/engagements/[id]/streams/[streamId]/process-model` — `process-model-service.ts` + `process-model-workspace.tsx` + `concept-picker.tsx`; seed, reorderable step cards w/ actor-unit chips, editable input bindings, add-step + create-method-with-typed-params, globals via ConceptPicker, live validation, generated-BPMN preview (`BpmnEditor`); linked from Process (BPMN) page. `tsc` clean; verified in-app (seed → 18 cards, delete round-trips). `delete_engagement` already cleans the new tables. Endpoints touching validation are async (await Fuseki). Phases in spec `2026-07-13-actor-method-process-model-design.md`.

- 2026-07-13 (Actor–Method process model redesign, Phase 1): New spec `docs/superpowers/specs/2026-07-13-actor-method-process-model-design.md` (Bobby-approved architecture: DB steps are source of truth → BPMN generated; params typed by ontology; first-class Actor tagged to a function unit). Phase 1 backend shipped + verified: 5 tables (Alembic `d5f8b2a1c3e4`), `process_model_router.py` (actors/methods/params/steps/globals + validate + generated BPMN), `process_validation.py` (dataflow engine — required inputs must bind to an earlier output or initialised global of a compatible ontology type, else unbound/unavailable/type_mismatch with correction suggestions), `process_model_bpmn.py`. Verified via API: missing-global and type-mismatch errors caught, fixes clear them, BPMN generated (lanes = actor function units). `delete_engagement` cleans the new tables. Phases 2 (seed importer from baselines) + 3 (workshop step-editor UI) remain. NOTE: shared Postgres — Bobby is actively using the app (a live "…University" education engagement exists), so API changes were kept additive/backward-compatible.
